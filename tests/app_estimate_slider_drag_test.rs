...
fn dragging_on_estimate_slider_updates_value() {
    ...
    app.handle_mouse_event(
        MouseEvent { kind: MouseEventKind::Down(MouseButton::Left), column: start_x, row: track.y, modifiers: crossterm::event::KeyModifiers::empty() },
        area,
    );
    let target_x = ui::slider_x_for_minutes(track, 0, 240, 5, 45);
    app.handle_mouse_event(
        MouseEvent { kind: MouseEventKind::Drag(MouseButton::Left), column: target_x, row: track.y, modifiers: crossterm::event::KeyModifiers::empty() },
        area,
    );
    ...
    // Close with OK
    app.handle_mouse_event(
        MouseEvent { kind: MouseEventKind::Down(MouseButton::Left), column: ok.x, row: ok.y, modifiers: crossterm::event::KeyModifiers::empty() },
        area,
    );
