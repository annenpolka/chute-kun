/**
 * 今日期限のサブタスクを取得するサンプルスクリプト
 *
 * このスクリプトは、Todoistから今日期限のタスクとそのサブタスク（期限問わず）を取得し、
 * 階層構造で表示します。
 */

import { createTodoistApi, getTodayTasksWithSubtasks, HierarchicalTask } from './lib/todoistClient';
...
async function main() {
  try {
    console.log('Todoistから今日期限のサブタスク情報を取得しています...');
    ...
    console.log('\n=== 取得した今日期限のタスク（階層構造） ===');
    console.log(`合計 ${todayTasksWithSubtasks.length} 個の親タスクが見つかりました\n`);

    // タスクを階層構造で表示
    displayTaskHierarchy(todayTasksWithSubtasks);
    ...
    countSubtasks(todayTasksWithSubtasks);
    console.log(`\n合計 ${subtaskCount} 個のサブタスクが見つかりました`);

    // 注意：サブタスクは期限に関わらず全て取得されます
    console.log('\n※ サブタスクは期限に関わらず、今日期限の親タスクに紐づくものが全て表示されています');
